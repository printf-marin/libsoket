...
// 代码定义了一个名为Resp的类，它有三个成员变量：整型的code、字符串型的msg和字符串型的msg。这个类还实现了与JSON序列化相关的方法。

// 代码又定义了一个名为LoginReq的类，它有四个成员变量：三个字符串型的username、password、salt和一个布尔型的login。该类还有一个成员函数CheckLogin()，用于检查登录是否成功。

// InitRouter()的函数用于初始化HTTP路由。其中定义了一些路由规则，如处理上传文件、登录和响应ping请求。还定义了一个静态文件的路由。

// 创建了一个HttpEngine对象，并通过InitRouter()函数初始化了路由。然后，输出了所有的路由信息，并设置了服务器的运行模式为调试模式。最后，调用Run()函数开始监听指定端口的HTTP请求。
